...
for i = 1 : max_iteration
    ...
    for k=1:m
        ...
        H_2 = z/q(k)/b;
        H_3 = hk/(I+bkn0);
        H_4 = -H_2*H_3*p(k)/log2(1+H_3*p(k))/(log2(1+H_3*p(k)))^2+...
            H_2/(log2(1+H_3*p(k)));
    ...
    %% 迭代更新拉格朗日乘子 lambda
    for k=1:m
        Grad_f(k) = p(k)*z*inv_pos(q(k)*b*log2(1+p(k)*hk/(I+bkn0)))+itr*kapa*D*c*f(k)^2 - erequirement(k);
    end
