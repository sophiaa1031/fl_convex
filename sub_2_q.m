...
%         V_2 = 2^(z/(b*(T_max(k)-itr*D*c/f(k))));
%         V_3 = p(k)*z/(q(k)^2*b*log2(1+p(k)*hk/(I+bkn0)));
%         l_q = -exp((2*(V_2/q(k)-1))*V_1)/q(k)^2-V_1*V_2*log(2)*2^(V_2/q(k)-1)*exp((2*V_2/q(k)-1)*V_1)/q(k)^3-mu(k)*V_3+beta;
...
%     V_2 = 2^(z/(b*(T_max(k)-itr*D*c/f(k))));
%     V_3 = p(k)*z/(q(k)^2*b*log2(1+p(k)*hk/(I+bkn0)));
%     l_q(k) = -exp(2*(V_2/q(k)-1)*V_1)/q(k)^2-...
%         V_1*V_2*log(2)*2^(V_2/q(k)-1)*exp((2*V_2/q(k)-1)*V_1)/q(k)^3-mu(k)*V_3+beta;
% end      
